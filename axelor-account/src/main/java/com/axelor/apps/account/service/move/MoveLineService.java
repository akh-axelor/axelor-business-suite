...
public class MoveLineService {
	 ...
	 * @param isDebit
	 * 		<code>true = debit</code>,
	 * 		<code>false = credit</code>
	 * @param date
	 * @param dueDate
	 * @param counter
	 * @param origin
	...
	/**
	 * Creating accounting move line method using all currency informations (amount in specific move currency, amount in company currency, currency rate)
	 * 
	 * @param move
	 * @param partner
	 * @param account
		...
		for (InvoiceLineTax invoiceLineTax : invoice.getInvoiceLineTaxList()){
			...
			if(companyTaxTotal.compareTo(BigDecimal.ZERO) != 0)  {
				...
				if(hasFixedAssets) {
					...
					moveLine = this.createMoveLine(move, partner, account, invoiceLineTax.getSubTotalOfFixedAssets(), invoiceLineTax.getCompanySubTotalOfFixedAssets(), null, !isDebitCustomer, invoice.getInvoiceDate(), null, moveLineId++, invoice.getInvoiceId(), null);
				...
				if(hasOtherAssets) {
					...
					moveLine = this.createMoveLine(move, partner, account, invoiceLineTax.getSubTotalExcludingFixedAssets(), invoiceLineTax.getCompanySubTotalExcludingFixedAssets(), null, !isDebitCustomer, invoice.getInvoiceDate(), null, moveLineId++, invoice.getInvoiceId(), null);
					moveLine.setTaxLine(invoiceLineTax.getTaxLine());
					moveLine.setTaxRate(invoiceLineTax.getTaxLine().getValue());
					moveLine.setTaxCode(tax.getCode());
					moveLines.add(moveLine);
