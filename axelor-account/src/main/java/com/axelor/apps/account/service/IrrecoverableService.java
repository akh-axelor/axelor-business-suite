...
public class IrrecoverableService{
	...
	@Inject
	public IrrecoverableService(AppAccountService appAccountService, SequenceService sequenceService, MoveService moveService, MoveLineService moveLineService, MoveLineRepository moveLineRepo,
		...
		if(irrecoverable.getInvoiceSet() != null && irrecoverable.getInvoiceSet().size() != 0)  {
			for(Invoice invoice : irrecoverable.getInvoiceSet())  {
				...
				try {
					...
					anomaly++;
					TraceBackService.trace(new AxelorException(e, e.getCategory(), I18n.get("Invoice")+" %s", invoice.getInvoiceId()), IException.IRRECOVERABLE, irrecoverable.getId());
		...
		if(irrecoverable.getPaymentScheduleLineSet() != null && irrecoverable.getPaymentScheduleLineSet().size() != 0)  {
			for(PaymentScheduleLine paymentScheduleLine : irrecoverable.getPaymentScheduleLineSet())  {
				...
				try {
					...
					anomaly++;
					TraceBackService.trace(new AxelorException(e, e.getCategory(), I18n.get(IExceptionMessage.IRRECOVERABLE_1), paymentScheduleLine.getName()), IException.IRRECOVERABLE, irrecoverable.getId());
	...
	public List<IrrecoverableReportLine> createIrrecoverableReportLineList(IrrecoverableInvoiceLine iil, Invoice invoice, BigDecimal prorataRate)  {
		...
		for(InvoiceLine invoiceLine : invoice.getInvoiceLineList())  {
			...
			irlList.add(this.createIrrecoverableReportLine(iil, invoiceLine.getName(), invoiceLine.getExTaxTotal().multiply(prorataRate).setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_EVEN), seq));
			seq++;
		}
		for(InvoiceLineTax invoiceLineTax : invoice.getInvoiceLineTaxList())  {
			irlList.add(this.createIrrecoverableReportLine(iil, invoiceLineTax.getTaxLine().getTax().getName(), invoiceLineTax.getTaxTotal().multiply(prorataRate).setScale(AppBaseService.DEFAULT_NB_DECIMAL_DIGITS, RoundingMode.HALF_EVEN), seq));
			seq++;
		}
